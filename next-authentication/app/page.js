...
async function getPostsServerSide() {
  await connectDB();
  try {
    // Fetch posts directly using Mongoose on the server for best performance
    const posts = await Post.find({}).sort({ createdAt: -1 }).lean(); 
    
    // Mongoose documents need to be serialized to plain objects for React components
...
export default async function Home() {
  const posts = await getPostsServerSide();

  return (
    <div className="container mx-auto px-4">
      {/* Header component includes sign-in/out and create-post links */}
      <Header /> 
      ...
      <div className="space-y-6">
        {posts.length > 0 ? (
          ...
          <p className="text-gray-500">No blog posts found. Be the first to create one!</p>
        )}
      </div>
    </div>
  );
